// D:\Projects\DoctorBooking\backend\routes\doctorRoutes.js
...
// Import payment guard middleware
const paymentGuard = require("../middleware/paymentGuard");

// Import email service for dual provider support
const emailService = require('../utils/emailService');

// Temporary doctor credentials
const DOCTORS = {
...
// ✅ GET DOCTOR PAYMENT STATUS (Public route - no guard needed)
...
// ✅ DOCTOR LOGIN - Check restriction status
router.post("/login", async (req, res) => {
  try {
    ...
    if (doctor) {
      ...
      console.log("Stored password type:", doctor.password.length > 30 ? "HASHED" : "PLAIN TEXT");
      ...
      if (doctor.password && doctor.password.length > 30) {
        ...
        console.log("🔑 Using direct compare for plain text password");
      ...
      if (isPasswordValid) {
        ...
        // Add warning headers if payment is overdue
        const response = {
          success: true,
          ...
          doctor: {
            ...
            lateFees: doctor.lateFees || 0,
            totalDue: (doctor.paymentStats?.totalCommissionPaid || 0) + (doctor.lateFees || 0)
          },
        };

        // Set warning headers if needed
        if (doctor.paymentStatus === 'overdue' || doctor.paymentStatus === 'late') {
          res.setHeader('X-Payment-Warning', 'Payment overdue');
          res.setHeader('X-Payment-Amount', (doctor.paymentStats?.totalCommissionPaid || 0) + (doctor.lateFees || 0));
        }
...
function getDoctorDataByEmail(email) {
  const doctors = {
    "sharma@doctor.com": {
      ...
      qrCodeUrl: "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=sharma@okhdfcbank&pn=Doctor&cu=INR",
    },
    "patel@doctor.com": {
      name: "Dr. Priya Patel",
      ...
      qrCodeUrl: "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=patel@okhdfcbank&pn=Doctor&cu=INR",
    },
    "kumar@doctor.com": {
      name: "Dr. Amit Kumar",
      ...
      qrCodeUrl: "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=kumar@okhdfcbank&pn=Doctor&cu=INR",
    },
    "gupta@doctor.com": {
      ...
      qrCodeUrl: "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=gupta@okhdfcbank&pn=Doctor&cu=INR",
    },
  };
...
// ✅ GET DOCTOR DASHBOARD STATS - With payment guard
router.get("/dashboard/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
    ...
    let appointments = await Appointment.find({
      $or: [
        ...
        { "doctor.name": `Dr. ${doctor.name.replace("Dr. ", "")}` },
...
// ✅ GET PENDING VERIFICATIONS - With payment guard
router.get("/pending-verification/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
    ...
    // Find all appointments waiting for verification
    ...
    console.log(`📋 Found ${pendingAppointments.length} pending verifications for ${doctor.name}`);
...
// ✅ VERIFY APPOINTMENT - Check if doctor can verify
  ...
  try {
    ...
    if (!canVerify) {
      return res.status(403).json({
        ...
        message: '⛔ Cannot verify appointments. Account restricted or payment overdue.',
    ...
    console.log(`✅ Appointment ${appointmentId} verified successfully by ${verifiedBy}`);
    ...
    if (doctor) {
      ...
      doctor.paymentStats.totalCommissionPaid += commissionAmount;
      doctor.paymentStats.totalPaymentsViaPlatform += 1;
      
      await doctor.save();
      console.log(`✅ Doctor's stats updated: +₹${doctorGets} earnings, +₹${commissionAmount} commission due`);
    }
...
router.get("/verification-stats/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
...
// ========== COMMISSION DUE with TOTAL from appointments ==========
router.get("/:doctorId/commission-due", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
    ...
    const totalDue = totalCommissionFromAllTime + (doctor.lateFees || 0);
    ...
    console.log("📊 Commission Due Calculation:", {
      ...
      monthlyCommission,
      storedCommissionPaid: doctor.paymentStats?.totalCommissionPaid,
    ...
    res.json({
      ...
      doctorName: doctor.name,
      doctorId: doctor.doctorId,
      commissionDue: totalCommissionFromAllTime,
      monthlyCommission,
      lastPaid: doctor.paymentStats?.lastCommissionPaid,
...
// ✅ PAY COMMISSION - Doctor pays commission to admin
router.post("/pay-commission", async (req, res) => {
  try {
    ...
    console.log(`✅ Commission payment recorded for ${doctor.name}: ₹${amount}`);
    ...
    try {
      ...
      console.error("❌ Failed to send confirmation email to doctor:", emailError);
    }
...
router.get("/payment-history/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
    ...
    const payments = await PaymentHistory.find({ doctorId })
      .sort({ paidAt: -1 });
    ...
    const totalCommission = appointments.reduce((sum, apt) => sum + (apt.amount * 0.01), 0);
...
// ✅ GET DOCTOR'S APPOINTMENTS - With payment guard
router.get("/appointments/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
...
// ✅ GET DOCTOR'S PATIENTS - With payment guard
router.get("/patients/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
...
// ✅ UPDATE APPOINTMENT STATUS - NO COMMISSION CHANGES!
router.patch("/appointments/:appointmentId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { appointmentId } = req.params;
...
// ✅ GET EARNINGS REPORT - With payment guard
router.get("/earnings/:doctorId", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
...
// ✅ UPDATE DOCTOR'S UPI DETAILS - With payment guard
...
// ✅ GET DOCTOR'S COMMISSION SUMMARY - With payment guard
router.get("/:doctorId/commission", paymentGuard.checkAccess, async (req, res) => {
  try {
    const { doctorId } = req.params;
    ...
    const totalCommission = (totalEarnings * doctor.commissionPercentage) / 100;
    ...
    res.json({
      ...
      paymentStatus: doctor.paymentStatus,
      lateFees: doctor.lateFees || 0,
      due: totalCommission - (doctor.paymentStats?.totalCommissionPaid || 0),
