...
export const AuthProvider = ({ children }) => {
  ...
  const checkDoctorAuth = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        try {
          const logoResponse = await axios.get(`/api/doctor/logo/${doctorData.id || doctorData.doctorId}`);
  ...
  const doctorLogin = async (email, password) => {
    try {
      ...
      if (response.data.success) {
        ...
        // 🆕 NEW: Fetch logo and clinic name after login
        try {
          const logoResponse = await axios.get(`/api/doctor/logo/${doctorData.id || doctorData.doctorId}`);
          if (logoResponse.data.success) {
            doctorData.logoUrl = logoResponse.data.logoUrl;
  ...
  return (
    <AuthContext.Provider value={{ 
      doctor, admin, loading,
      doctorLogin, doctorLogout,
